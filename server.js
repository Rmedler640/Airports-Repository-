...
async function callWithRetry(fn, retries = 5, delayMs = 4000) {
...
app.post('/api/guess', async (req, res) => {
  try {
    ...
    // Strip leading K (ICAO prefix) so KHOU matches HOU
    const gStripped = g.startsWith('K') && g.length === 4 ? g.slice(1) : g;
    const correct =
      g === airport.code ||
      gStripped === airport.code ||
...
app.listen(PORT, async () => {
  await initDB();
  // Block startup until hints are generated — retries handle cold starts
  console.log('[startup] Pre-generating daily hints...');
  try {
    const todayKey = getTodayKey();
    const data = await getDailyData(todayKey);
    const fallbacks = data.airports.filter(a => isFallbackHint(a.hint)).length;
    if (fallbacks > 0) {
      console.log(`[startup] ${fallbacks} fallback hints — regenerating now...`);
      const fresh = await buildDailyCache(todayKey, true);
      const stillFallback = fresh.airports.filter(a => isFallbackHint(a.hint)).length;
      console.log(`[startup] Regen done: ${5 - stillFallback}/5 real hints`);
    } else {
      console.log('[startup] All hints OK');
    }
  } catch (e) {
    console.error('[startup] Hint pre-gen failed:', e.message);
  }
